...
import { ArrowLeft, Truck, Plus, Phone, Mail, MapPin, PackagePlus, CreditCard } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-[12px] border border-gray-200 p-4">
          ...
          <p className="font-display text-xl font-bold text-agro-dark mt-1">{formatCurrency(supplier.totalDelivered)}</p>
        </div>
        <div className="bg-white rounded-[12px] border border-gray-200 p-4">
          <p className="text-xs text-muted font-body uppercase tracking-wide">Total Paid</p>
          <p className="font-display text-xl font-bold text-green-600 mt-1">{formatCurrency(supplier.totalPaid)}</p>
        </div>
        <div className="bg-white rounded-[12px] border border-gray-200 p-4">
          <p className="text-xs text-muted font-body uppercase tracking-wide">Balance Owed</p>
          <p className={cn("font-display text-xl font-bold mt-1", supplier.balance > 0 ? "text-red-600" : "text-green-600")}>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex gap-1 bg-gray-100 p-1 rounded-[10px] w-fit">
            {(["DELIVERIES", "PAYMENTS"] as Tab[]).map((t) => (
              <button key={t} onClick={() => setTab(t)}
                className={cn("px-4 h-9 rounded-[8px] font-body text-sm font-medium transition-colors",
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowPayment(true)}
              className="flex items-center gap-1.5 h-9 px-3 rounded-[8px] border border-gray-200 text-agro-dark font-body text-sm hover:bg-gray-50 transition-colors">
              <CreditCard className="h-4 w-4" /> Record Payment
            </button>
            <button onClick={() => setShowDelivery(true)}
              className="flex items-center gap-1.5 h-9 px-3 rounded-[8px] bg-primary text-white font-body text-sm hover:bg-primary-dark transition-colors">
              <PackagePlus className="h-4 w-4" /> Record Delivery
            </button>
          </div>
        </div>
          ...
          supplier.payments.length === 0 ? (
            ...
            <div className="bg-white rounded-[12px] border border-gray-200 shadow-card overflow-hidden">
              <table className="w-full text-sm hidden md:table">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    {["Amount", "Method", "Reference", "For Delivery", "Date"].map((h) => (
                      <th key={h} className="text-left px-4 py-3 font-body text-xs font-semibold text-muted uppercase tracking-wide">{h}</th>
                    ))}
                  </tr>
                </thead>
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/dashboard/suppliers/${supplierId}/payments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount: form.amount, method: form.method, reference: form.reference || undefined, notes: form.notes || undefined, deliveryId: form.deliveryId || undefined }),
      });
      const data = await res.json();
      if (data.success) onSaved();
      else setError(data.error ?? "Failed to save");
